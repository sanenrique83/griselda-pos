...
- **`opciones_modificador.foto_url TEXT` (nullable)** — nuevo campo, agregado a la migración `20260801000025` (sigue sin aplicar). Antes solo el producto completo tenía imagen; ahora cada opción/guisado puede tener la suya. En Catálogo, el editor de opción de modificador tiene un campo de imagen opcional que **reutiliza `subirImagenProducto()`** (mismo bucket "productos", sin duplicar lógica de subida).
...
- Mismo aviso de despliegue que la ronda anterior: hasta que se aplique la migración, las queries que ya seleccionan `foto_url` de `opciones_modificador` (`cargarModificadores`, `cargarGuisados`) fallarán contra la base real.
