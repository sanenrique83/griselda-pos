...
- **Layout de tarjeta horizontal**: la cuadrícula de 2 columnas con imagen arriba (alta, pocas visibles) pasó a una lista de una columna con filas horizontales — thumbnail cuadrado (56px) a la izquierda, nombre/precio/badge "Agotado" al centro, stepper a la derecha. Caben muchas más opciones sin scroll. Nota de honestidad: el archivo `docs/mockups/09-captura-rapida.png` en sí, al revisarlo de nuevo, en realidad muestra tarjetas verticales (imagen arriba) — igual que ya estaba construido. El cambio a horizontal se hizo por la descripción explícita de Rober y su razón (más opciones visibles sin tanto scroll), no porque el mockup lo mostrara así; vale la pena que lo confirme visualmente.
- **Botón "Filtros"**: se agregó junto al buscador (ícono sliders). No es decorativo — controla un filtro real ("ocultar agotados", sobre `disponible`, que ya venía de `insumos.disponible`), no una función inventada.
- **Chips de categoría — diagnóstico**: el código de la ronda anterior (`grupos.length > 1`, `cargarGuisados` agrupando por `mostrar_en_rapido`) está correcto y sin bugs — se confirmó consultando la base real. La causa de que no aparecieran para "Tacos" (producto id 11, el mismo nombre que usa el mockup) es que solo tiene **un** grupo activo con `mostrar_en_rapido=true` ("Guisados"); los otros dos que existen ("Barbacoa", "extras") están con `activo=false` en Catálogo. Con un solo grupo activo, el filtro es redundante a propósito y el guard lo oculta — comportamiento esperado, no un bug. La causa raíz de que el resumen de la ronda pasada dijera "ya están conectados" sin que se vieran: se verificó la lógica del código pero nunca se probó contra los datos reales de este producto específico, donde solo hay 1 grupo activo en vez de los 3 que sugiere el mockup. No se tocaron los flags `activo` de "Barbacoa"/"extras" en la base — es una decisión de contenido del admin, no algo para cambiar sin confirmación; si se activan esos grupos (o se crean más con `mostrar_en_rapido`), los chips aparecerán solos, sin tocar código.
