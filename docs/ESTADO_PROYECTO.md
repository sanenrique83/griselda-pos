...
| **Administración de usuarios (`/mas/usuarios`)** | ⚪ Implementado, migración sin aplicar todavía (`perfiles.telefono`/`fecha_contratacion` — `activo` ya existía desde el esquema inicial). Crear usuario usa `lib/supabase/admin.ts` (cliente service_role) — **requiere agregar la variable de entorno `SUPABASE_SERVICE_ROLE_KEY` en `.env.local` y en Vercel antes de que "Nuevo usuario" funcione**; sin ella, `crearUsuario()` devuelve un error controlado en vez de tronar. Enlaza a `/mas/asistencia?usuarioId=<id>` en vez de duplicar esa pantalla. El PIN rápido (próxima feature) debe integrarse aquí mismo, no aparte. |
