...
- **⚠️ Duda de diseño sin resolver — toggle "Imprimir automáticamente"**: antes vivía fijo en el pie pegado al botón "Cobrar" (decisión deliberada de una ronda anterior, documentada en un comentario: "pegada al botón... para que nunca se pierda de vista sin hacer scroll"). Se movió al cuerpo scrolleable para coincidir con la posición exacta del mockup (como tarjeta blanca normal, antes de la tarjeta de error). Esto reabre esa decisión anterior — falta que Rober confirme si prefiere mantenerlo en el cuerpo (fiel al mockup) o regresarlo al pie fijo (siempre visible sin scroll).
- Todos los montos migrados a `formatCurrency()` en vez de `.toFixed(2)` suelto.

**Pendiente de Bloque F**: migrar el resto de pantallas (Historial, Dashboard, Más, etc.) a Boton/Sheet/Tarjeta + verde bosque, una por una.
