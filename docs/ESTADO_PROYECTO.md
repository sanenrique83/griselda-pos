...
**Corrección — control real de estilo_visual por grupo (ya no inferido por conteo de opciones).** Rober pidió reemplazar la heurística (≤5 opciones → tarjetas, >5 → lista) por un campo real: `grupos_modificadores.estilo_visual TEXT NOT NULL DEFAULT 'cajas' CHECK (IN ('cajas','lista'))` — migración `20260801000025`, **sin aplicar**. Selector "Cajas/Lista" agregado al form de grupo en Catálogo (`SeccionProductos.tsx`). Los 3 layouts viejos (tarjetas en fila con scroll horizontal, lista de radio, grid de 2 columnas) se redujeron a 2: 'cajas' = grid de 4 columnas que siempre envuelve dentro del ancho de pantalla (nunca scroll horizontal, aplica tanto a selección única como múltiple), 'lista' = filas de ancho completo (sin cambios de comportamiento, ahora también soporta selección múltiple con cuadrado en vez de círculo). La lógica círculo/cuadrado del indicador (según `grupo.maximo`) es independiente del estilo y no se tocó.
