...
- **Badge "Cerrada" — el único estado real de los 3 del mockup**: `cargarRecibos()` solo carga `movimientos_caja` con `tipo='cobro'` — es decir, absolutamente todo lo que se muestra aquí ya está cerrado/cobrado por definición de la query. "Abierta" (pedido aún abierto, sin movimiento de caja) y "Cancelada" (existe como `tipo='cancelacion'` en `movimientos_caja`, pero la query actual los excluye con `.eq('tipo','cobro')`) no se construyeron: "Cancelada" sería una extensión real y acotada (agregar `tipo IN ('cobro','cancelacion')` a la query), pero "Abierta" mezclaría datos de pedidos en vivo (dominio de `/pedidos`) dentro de lo que hoy es un ledger de movimientos ya cerrados — se dejó fuera de esta ronda "puramente visual" y anotado aquí para que Rober decida si lo quiere como tarea aparte.
